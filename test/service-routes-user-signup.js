describe('User Signup Services ', function() {
  ...
  describe('Create a new user', function() {
    it('success', function(done) {
      client.post('/api/v1/user', {'username':'user2','name':'user2','phoneNumber':'15063604184','email':'user2@user2.com','password':'user2','vPassword':'user2'}, function(err, req, res, data) {
        if (err) {
          throw new Error(JSON.stringify(err));
        }
        else {
  ...
  describe('Create a new user', function() {
    it('Non-matching passwords', function(done) {
      client.post('/api/v1/user', {'username':'user3','name':'user3','phoneNumber':'15063604185','email':'user3@user3.com','password':'user3','vPassword':'user2'}, function(err, req, res, data) {
        if (err) {
  ...
  describe('Create a new user', function() {
    it('Invalid email', function(done) {
      client.post('/api/v1/user', {'username':'user3','name':'user3','phoneNumber':'15063604185','email':'user3','password':'user3','vPassword':'user3'}, function(err, req, res, data) {
        if (err) {
          data.message.should.include('Email address must be valid');
          //assert.equal(data.message, "Email address must be valid");
          done();
        }
        else {
          throw new Error('invalid response from /api/v1/user ' + JSON.stringify(data));
